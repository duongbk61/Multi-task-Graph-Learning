...
def train_step(model, optimizer, ponzi_loader, phish_loader, args, device):
    ...
    for batch_ponzi, batch_phish in zip(ponzi_loader, phish_loader):
        ...
        # Forward pass
        # Ponzi Head task
        out_ponzi_p, out_phish_p, loss_co_p = model(batch_ponzi.x_dict_new, batch_ponzi.edge_index_dict)
        size_ponzi = batch_ponzi['CA'].batch_size
        loss_ponzi = F.cross_entropy(out_ponzi_p[:size_ponzi], batch_ponzi['CA'].y[:size_ponzi])
        
        # Phish Head task
        out_ponzi_h, out_phish_h, loss_co_h = model(batch_phish.x_dict_new, batch_phish.edge_index_dict)
...
def evaluate(model, loader, target_node, task_type, args, device):
    ...
    for batch in loader:
        ...
        out_ponzi, out_phish, loss_co = model(batch.x_dict_new, batch.edge_index_dict)
