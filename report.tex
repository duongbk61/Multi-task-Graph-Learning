...
\textit{(i)} a pre-trained Interaction-aware Conditional Variational
Autoencoder (ICVAE) that generates stochastic multi-view node-feature
augmentations conditioned on meta-interaction type, enriching node
representations for multi-view contrastive learning;
...
Unlike oversampling approaches, our ICVAE does not synthesise new
nodes; instead it generates $K$ stochastic feature views per
\emph{existing} node, conditioned jointly on the source node features
and meta-interaction type.  These views serve as structured
anchor/positive/negative samples for contrastive self-supervision,
enriching the representation of low-activity nodes that would
otherwise be underrepresented in the feature space.
...
To enrich node representations through stochastic multi-view feature
augmentation, we employ a pre-trained Interaction-aware Conditional
Variational Autoencoder (ICVAE)~\cite{li2022graphvae}.
Unlike oversampling methods that create additional minority-class
nodes, ICVAE generates $K$ stochastic feature views for every
existing node by sampling from a learned latent distribution
conditioned on source node features and meta-interaction type.
This augmentation strategy does not alter the class distribution
directly; rather, it produces richer, more varied representations
that improve contrastive self-supervision and reduce feature-space
variance for nodes with low interaction activity.
...
\subsection{Neuro-Symbolic Integration (Decision-Tree-Distilled Rules)}
\label{sec:expert}

To inject interpretable symbolic priors into the neural pipeline, we
distill a set of lightweight decision rules from the data rather than
hand-crafting them. Concretely, we fit a shallow Decision Tree
(maximum depth $3$) on the \emph{training split only}, using the same
raw node features consumed by the GNN, and extract the root-to-leaf
decision paths as boolean scoring functions $s^P$ (Ponzi) and $s^H$
(phishing). Because the rules are induced solely from training-set
statistics and never observe validation or test labels, this procedure
introduces \emph{no information leakage}.

The node features are raw on-chain interaction statistics rather than
normalized quantities, so the extracted thresholds correspond to
real-valued transaction magnitudes. Following the handcrafted feature
construction of the baseline~\cite{paper_baseline_2408.00641}, each
account is described by $14$ features---seven per interaction channel
(call and transfer)---comprising, per channel: a \emph{balance}
(total investment minus total return), total investment, total return,
initiation and reception counts, and average investment and return.
The \emph{balance} features are signed by construction: empirically
$4$--$12\%$ of accounts exhibit negative balances (received more than
they sent). Consequently, thresholds of the form $x > {-}10$ on a
balance feature are meaningful split points rather than vacuous guards.
For instance, in the phishing data the average call balance is $-1038$
for legitimate accounts versus $-0.36$ for phishing accounts, so a
lower bound near zero effectively separates phishing wallets (whose net
call balance is close to zero) from legitimate accounts (whose balance
is strongly negative). We verified this column semantics directly from
the data via the algebraic identities $\textit{total}=\textit{count}\times\textit{average}$
and $\textit{balance}=\textit{total inv.}-\textit{total ret.}$, which
hold to numerical precision.

\medskip\noindent\textbf{Ponzi score.}
A CA node is flagged by rule $s^P\!\in\!\{0,1\}$ if it
matches a distilled Ponzi redistribution branch:
\begin{equation}
  s^P = \mathbf{1}\!\left[
...
\mathbf{x}^{CA}_{11}$ denote call balance, call reception count,
transfer balance, and transfer reception count respectively.

\medskip\noindent\textbf{Phishing score.}
An EOA node is flagged by rule $s^H\!\in\!\{0,1\}$ under two
complementary distilled branches:
\begin{equation}
  s^H = \mathbf{1}\!\left[
  ...
  \right],
\end{equation}
where $\mathbf{x}^{EOA}_{8},\mathbf{x}^{EOA}_{7},\mathbf{x}^{EOA}_{0}$
denote transfer total investment, transfer balance, and call balance
respectively, and the whale condition $s_{\mathrm{whale}}$ captures
high-investment accounts with zero call activity (call balance, call
total return, and call average return all equal to zero)---a common
pattern among dormant phishing wallets.
...
\item \textbf{Loss mode}: rule scores serve as soft targets in an
...
\medskip\noindent\textbf{Standalone rule evaluation.}
To quantify the intrinsic signal carried by the distilled symbolic
priors independently of the GNN, we evaluate each rule set as a
standalone classifier on the test split, reporting precision, recall,
and coverage in Table~\ref{tab:rules}. This isolates the contribution
of the symbolic component from the learned neural representations.

\begin{table}[h]
\centering
\caption{Standalone evaluation of the distilled rules on the test
split. Coverage denotes the fraction of nodes flagged positive by the
rule.}
\label{tab:rules}
\begin{tabular}{@{}lccc@{}}
\toprule
Rule set & Precision & Recall & Coverage \\
\midrule
Ponzi ($s^P$)    & -- & -- & -- \\
Phishing ($s^H$) & -- & -- & -- \\
\bottomrule
...
The ICVAE generates one augmented feature view \emph{per
meta-interaction type}: $\mathbf{z}^{(1)}$ is conditioned on
\emph{call}-type meta-interactions ($r_{\mathrm{call}}$), while
$\mathbf{z}^{(2)}$ and $\mathbf{z}^{(3)}$ are conditioned on two
distinct \emph{transfer}-type meta-interactions
($r_{\mathrm{trans}}^{(1)}$ and $r_{\mathrm{trans}}^{(2)}$).

From a domain perspective, call interactions and transfer interactions
reflect fundamentally different account behaviours on Ethereum:
a contract call represents programmatic function invocation, whereas
a value transfer represents a direct asset movement.
Consequently, the representations derived from the two transfer-type
views, $\mathbf{z}^{(2)}$ and $\mathbf{z}^{(3)}$, should be more
similar to each other than to the call-type view $\mathbf{z}^{(1)}$.

Based on this semantic principle~\cite{paper_baseline_2408.00641},
for each node type we construct a triplet:
\begin{equation}
  \mathbf{a} = \mathbf{z}^{(2)},\quad
  \mathbf{p} = \mathbf{z}^{(3)},\quad
  \mathbf{n} = \mathbf{z}^{(1)},
\end{equation}
where $\mathbf{z}^{(2)}$ (transfer-type) is the anchor,
$\mathbf{z}^{(3)}$ (transfer-type) is the positive sample, and
$\mathbf{z}^{(1)}$ (call-type) is the negative sample.
The triplet contrastive loss is:
\begin{equation}
...
capture the semantic distinction between call-based and transfer-based
interaction patterns, thereby improving the framework's ability to
distinguish diverse account behaviour profiles.
