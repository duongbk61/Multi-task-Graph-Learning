...
normalized node features consumed by the GNN, and extract the
root-to-leaf decision paths as boolean scoring functions $s^P$ (Ponzi)
and $s^H$ (phishing). Because the rules are induced solely from
training-set statistics and never observe validation or test labels,
this procedure introduces \emph{no information leakage}.

Since the input features are pre-normalized, the extracted thresholds
operate in the normalized feature space and should be interpreted as
learned split points rather than raw on-chain quantities. Some
branches (e.g.\ conditions of the form $x > -10$) act as near-trivial
guards that the tree retains but which contribute little discriminative
power; we keep them for fidelity to the distilled tree.
